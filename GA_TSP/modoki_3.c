...
int world_stars;
int **probability_table[2];
RealNumber *entropy_table_a;
RealNumber *entropy_table_c;
RealNumber *entropy_table_p;
...
int initialize_entropy_table(int num_parent)
{
    ...
    ret_code = 0;
    if(entropy_table_c == NULL || entropy_table_p == NULL)
    {
        ret_code = -1;
        goto error01;
    }
    ...
    for(i=1;i<num_parent;i++)
    {
        ...
        temp_v[i] = -log(temp_s);
        temp_s *= temp_v[i];
        entropy_table_a[i] = temp_s;
    }
    temp_v[0] = 0.0;
    entropy_table_a[0] = 0.0;
    temp_v[num_parent] = 0.0;
    entropy_table_a[num_parent] = 0.0;
    for(i=0;i<num_parent;i++)
    {
        entropy_table_c[i] = temp_v[i+1];
        entropy_table_p[i] = temp_v[i];
    }
    entropy_table_c[num_parent] = 0.0;
    entropy_table_p[num_parent] = 0.0;
...
int initialize_probability_table(void)
{
    int i,j,ret_code;
    ret_code = 0;
    if(probability_table[0] == NULL || probability_table[0][0] == NULL)
    {
        ret_code = -1;
        goto error01;
    }
    ...
    {
        probability_table[0][0][i] = 0;
    }
    for(i=0;i<world_stars*world_stars;i++)
    {
        probability_table[1][0][i] = 0;
    }
    error01:
    return ret_code;
}
...
int remove_probability_table(int *gene)
{
    int i,ret_code;
    ret_code = 0;
    if(probability_table[0] == NULL || probability_table[0][0] == NULL || gene  == NULL)
    {
        ret_code = -1;
        goto error01;
    }
    ...
    {
        probability_table[0][gene[i]][gene[i+1]]--;
        probability_table[0][gene[i+1]][gene[i]]--;
    }
    probability_table[0][gene[0]][gene[world_stars-1]]--;
    probability_table[0][gene[world_stars-1]][gene[0]]--;

    for(i=0;i<world_stars-2;i++)
    {
        probability_table[1][gene[i]][gene[i+2]]--;
        probability_table[1][gene[i+2]][gene[i]]--;
    }
    probability_table[1][gene[0]][gene[world_stars-2]]--;
    probability_table[1][gene[world_stars-2]][gene[0]]--;
    probability_table[1][gene[1]][gene[world_stars-1]]--;
    probability_table[1][gene[world_stars-1]][gene[1]]--;
...
int evaluate_entropy(int **gene,double *entropy,int size)
{
    ...
    for(i=0;i<size;i++)
    {
        for(j=0;j<world_stars-1;j++)
        {
            temp += entropy_table_a[probability_table[0][gene[i][j+1]][gene[i][j]]];
        }
        temp += entropy_table_a[probability_table[0][gene[i][0]][gene[i][world_stars-1]]];
    }
    for(i=0;i<size;i++)
    {
        for(j=0;j<world_stars-2;j++)
        {
            temp += entropy_table_a[probability_table[1][gene[i][j+2]][gene[i][j]]];
        }
        temp += entropy_table_a[probability_table[1][gene[i][0]][gene[i][world_stars-2]]];
        temp += entropy_table_a[probability_table[1][gene[i][1]][gene[i][world_stars-1]]];
    }
    *entropy = temp;
    return ret_code;
}

int evaluate_diff_entropy_c(int **gene,RealNumber *entropy,int size)
{
    int i,j,ret_code;
    RealNumber temp;
    ret_code = 0;
    ...
    for(i=0;i<size;i++)
    {
        ...
        for(j=0;j<world_stars-1;j++)
        {
            temp += entropy_table_c[probability_table[0][gene[i][j+1]][gene[i][j]]];
        }
        temp += entropy_table_c[probability_table[0][gene[i][0]][gene[i][world_stars-1]]];
        for(j=0;j<world_stars-1;j++)
        {
            temp += entropy_table_c[probability_table[1][gene[i][j+2]][gene[i][j]]];
        }
        temp += entropy_table_c[probability_table[1][gene[i][0]][gene[i][world_stars-2]]];
        temp += entropy_table_c[probability_table[1][gene[i][1]][gene[i][world_stars-1]]];
        entropy[i] = temp;
    }
    return ret_code;
}

int evaluate_diff_entropy_p(int **gene,RealNumber *entropy,int size)
{
    int i,j,ret_code;
    RealNumber temp;
    ret_code = 0;
    ...
    for(i=0;i<size;i++)
    {
        ...
        for(j=0;j<world_stars-1;j++)
        {
            temp += entropy_table_p[probability_table[0][gene[i][j+1]][gene[i][j]]];
        }
        temp += entropy_table_p[probability_table[0][gene[i][0]][gene[i][world_stars-1]]];
        for(j=0;j<world_stars-1;j++)
        {
            temp += entropy_table_p[probability_table[1][gene[i][j+2]][gene[i][j]]];
        }
        temp += entropy_table_p[probability_table[1][gene[i][0]][gene[i][world_stars-2]]];
        temp += entropy_table_p[probability_table[1][gene[i][1]][gene[i][world_stars-1]]];
        entropy[i] = temp;
    }
    return ret_code;
}
...
int main()
{
    ...
    entropy_table_c = (RealNumber *)malloc(3*(num_parent+1)*sizeof(RealNumber));
    if(entropy_table_c == NULL)
    {
        ret_code = -1;
        goto error08;
    }
    entropy_table_p = entropy_table_c + num_parent+1;
    entropy_table_a = entropy_table_p + num_parent+1;
    ...
    probability_table[0] = (int **)malloc(2*world_stars * sizeof(int *));
    if(probability_table[0] == NULL)
    {
        ret_code = -1;
        goto error11;
    }
    probability_table[0][0] = (int *)malloc(2*world_stars*world_stars*sizeof(int));
    if(probability_table[0][0] == NULL)
    {
        ret_code = -1;
        goto error12;
    }
    for(i=1;i<2*world_stars;i++)
    {
        probability_table[0][i] = probability_table[0][i-1] + world_stars;
    }
    probability_table[1] = probability_table[0] + world_stars;
    ...
    for(i=0;i<num_parent;i++)
    {
        if(i % m == 0)
        {
            evaluate_diff_entropy_c(child_gene,child_entropy,num_child);
    ...
    evaluate_entropy(parent_gene,&Entropy,num_parent);
    t2 = 0.0;
    for(i=0;i<num_parent;i++)
    {
    ...
    while(1)
    {
        evaluate_diff_entropy_p(parent_gene,parent_entropy,num_parent);
        for(k=0;k<num_parent;k++)
        {
            parent_score[k] = parent_distance[k] - Temperature * parent_entropy[k];
        }
        sort(index,parent_score,num_parent);
        ...
        for(j=0;j<num_parent;j++)
        {
            if(j % m == 0)
            {
                ...
                for(k=0;k<num_child;k++)
                {
                    child_score[k] = child_distance[k] - Temperature * child_entropy[k];
                }
            }

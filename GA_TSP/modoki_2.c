...
int initialize_entropy_table(int num_parent)
{
    ...
    for(i=1;i<num_parent;i++)
    {
        ...
        temp_v[i] = -log(temp_s);
        temp_s *= temp_v[i];
        entropy_table_p[i] = temp_s;
    }
    temp_v[0] = 0.0;
    entropy_table_p[0] = 0.0;
...
int evaluate_entropy(int **gene,double *entropy,int size)
{
    ...
    for(i=0;i<size;i++)
    {
        for(j=0;j<world_stars-1;j++)
        {
            temp += entropy_table_p[probability_table[gene[i][j+1]][gene[i][j]]];
        }
        temp += entropy_table_p[probability_table[gene[i][0]][gene[i][world_stars-1]]];
    }
    *entropy = temp;
    return ret_code;
}
...
int evaluate_diff_entropy(int **gene,RealNumber *entropy,int size)
{
    ...
    for(i=0;i<size;i++)
    {
        ...
        for(j=0;j<world_stars-1;j++)
        {
            temp += entropy_table_c[probability_table[gene[i][j+1]][gene[i][j]]];
        }
        temp += entropy_table_c[probability_table[gene[i][0]][gene[i][world_stars-1]]];
        entropy[i] = temp;
    }
    return ret_code;
}
...
int main()
{
    ...
    entropy_table_c = (RealNumber *)malloc(2*(num_parent+1)*sizeof(RealNumber));

...
int progress_generation(int **gene,int *index)
{
    ...
    for(i=0;i<num_parent/2;i++)
    {
        for(j=0;j<world_stars;j++)
        {
            urnd[gene[index[i]][j]] += j*world_stars;
        }
    }
    urnd[0] = 0;
    I_sort_p(urnd,temp_gene[4],temp_list[0],NULL,world_stars);
    for(i=5;i<num_parent;i++)
    {
        ...
        else if(urnd[0] < 50)
        {
            ...
            for(j=0;j<world_stars;j++)
            {
                temp_list[0][gene[index[urnd[0]]][j]] = j;
            }
            l = (world_stars < 5)? world_stars : 5;
            get_random(urnd+1,l+1);
            urnd[1] %= world_stars;
            k = 10;
            for(j=urnd[1];j<world_stars;j++)
            {
                temp_list[0][gene[index[urnd[0]]][j]] += (urnd[2+l] % k) - 5;
                l--;
            }
            for(j=0;j<world_stars;j++)
            {
            ...
            urnd[0] = 0;
            for(j=1;j<world_stars;j++)
            {
                urnd[j] = temp_list[0][j];
            }
            I_sort_p(urnd,temp_gene[i],temp_list[1],NULL,world_stars);
        }
        else
        {
...
int main()
{
    ...
    sort(index,diff);
    t2 = diff[index[(3*num_parent)/4]]/entropy_table[1];
    ...
    sort(index,diff);
    k = 0;
    while(entropy[index[0]] > 20.0 || k < 800)
    {
        evaluate_score(gene,diff);
        evaluate_entropy(gene,entropy);
        for(j=0;j<num_parent;j++)
    ...
    while(flag)
    {
        ...
        for(i=0;i<world_stars;i++)
        {
            for(j=i+2;j<world_stars;j++)
            {
                ...
                m = 0;
                t1 = distance_table[gene[index[0]][q[0]]][gene[index[0]][q[0]]];
                t1 += distance_table[gene[index[0]][q[2]]][gene[index[0]][q[3]]];
                for(k=1;k<2;k++)
                {
                    t2 = distance_table[gene[index[0]][q[0]]][gene[index[0]][q[2]]];
                    t2 += distance_table[gene[index[0]][q[1]]][gene[index[0]][q[3]]];
                    if(t1 > t2)
                    {
                        t1 = t2;
                        m = k;
                ...
                if(m != 0)
                {
                    ...
                    for(n=q[1];n<=q[2];n++)
                    {
                        gene[num_parent][l-n] = gene[index[0]][n];
                    }
                    for(n=q[1];n<=q[2];n++)
                    {
                        gene[index[0]][n] = gene[num_parent][n];
                    }
                    flag = 1;
                }
            }
    ...
    for(i=0;i<world_stars-1;i++)
    {
        t1 += distance_table[gene[index[0]][i+1]][gene[index[0]][i]];
    }
    t1 += distance_table[gene[index[0]][0]][gene[index[0]][world_stars-1]];
    printf("%f\n",t1);

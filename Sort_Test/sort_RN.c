...
#ifdef Merge
int sort(RealNumber *input_data,int size)
{
    int i,j,k,l,m,n,m_max,n_max,ret_code;
    RealNumber *data[2];
    ret_code = 0;
    data[0] = input_data;
    data[1] = (RealNumber *)malloc(size*sizeof(RealNumber));
    if(data[1] == NULL)
    {
        ret_code = -1;
        goto error01;
...
int sort(RealNumber *input_data,int size)
{
    ...
    for(j=0;j<size;j++)
    {
        if(data[0][j] & (1 << (BitSize - 1)))
        {
            data[1][j] = data[0][j] ^ ~0;
        }
        else
        {
            data[1][j] = data[0][j] ^ (1 << (BitSize - 1));
        }
    }

    k = 1;
    for(i=0;i<(sizeof(RealNumber)/Charlie);i++)
    {
        for(j=0;j<=Alpha;j++)
        {
            v[j] = 0;
    ...
    for(j=0;j<size;j++)
    {
        if(data[k][j] & (1 << (BitSize - 1)))
        {
            data[k^0x01][j] = data[k][j] ^ (1 << (BitSize - 1));
        }
        else
        {
            data[k^0x01][j] = data[k][j] ^ ~0;
        }
    }
    k ^= 0x01;
...
int sort(RealNumber *input_data,int size)
{
    ...
    i = 0;
    while(isnan( *(RealNumber *)(data[l] + i) ))
    {
        i++;
    }
    k = 0;
    ...
    if(i + k != 0)
    {
        ...
        while(k < i)
        {
            data[l^0x01][j++] = data[l][k++];
        }
        while(k < a)
        {
            data[l^0x01][i++] = data[l][k++];
        }
        while(k < size)
        {
            data[l^0x01][j++] = data[l][k++];
...
{
    if(*(RealNumber *)a > *(RealNumber *)b)
    {
        return 1;
    }
    else if(*(RealNumber *)a < *(RealNumber *)b)
    {
        return -1;
    }
    else
...
int sort(RealNumber *input_data,int size)
{
    int ret_code;
    ret_code = 0;
    qsort(input_data,size,sizeof(RealNumber),comp);
    return ret_code;
}
#endif
...
int main()
{
    ...
    data = (RealNumber *)malloc(file_stat.st_size);
    if(data == NULL)
    {
        ret_code = -1;
        goto error02;
    ...
    if(file_size != file_stat.st_size || file_size % (Times*sizeof(RealNumber)) != 0)
    {
        ret_code = -1;
        goto error02;
    }
    ...
    i = file_size/(Times*sizeof(RealNumber));
    for(j=0;j<Times;j++)
    {
        sort(data + i*j,i);
    }

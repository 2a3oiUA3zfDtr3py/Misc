...
#define num_group_size 20
#define num_data_set 24
...
void* calc_subset(void *arg) {
    ...
    int i,j,k,l,q;
    uint64_t *urnd;
    RealNumber ***a,**b,*c,*diff,*calc_gene,temp_diff;
    double *temp_gene[2];
    ...
    for(l=0;l<num_group_size;l++)
    {
        ...
        if(temp_gene[1][element_size] < 0.8)
        {
...
{
    int i,j,k,l,m,n,ret_code,grupe_num;
    ...
    for(i=0;i<grupe_num;i++)
    {
        ...
        if(m==-1)
        {
            ...
            for(j=0;j<num_group_size;j++)
            {
                ...
                save_gene(file_name,superset_gene[superset_idx[grupe_num*j + thread_grupe_num[n]]]);
            }
        }
        for(j=0;j<num_group_size;j++)
        {
...
int main()
{
    int i,j,k,ret_code,*superset_idx,*file_index;
    ...
    for(i=0;i<num_parent;i++)
    {
        printf("%f\n",superset_diff[i]/((RealNumber)num_data_set));
    }
    printf("\n%f\n\n\n",data_diff[0]);

...
#endif
#define num_generate 800
#define num_parent 80
#define num_data_set 1
#define data_pool_size 108
/*
...
int calc(RealNumber *gene,RealNumber **data,int width,int height,RealNumber *diff,RealNumber ***a,RealNumber **b,RealNumber *c) {
        ...
        for(j=0;j<height;j++) {
            for(k=0;k<width;k++) {
                ...
                for(n=0;n<matrix_size[i][1];n++) {
                    ...
                    if(c[pix_ref*hidden_channels+n] < 0.0) {
                        b[i+1][matrix_size[i][1]*(j*width+k)+n] = 0.0;
                    }else{
    ...
    for(j=-1;j<=1;j++) {
        for(k=-1;k<=1;k++) {
            if(j!=0 || k!=0) {
                p[l][0] = j*matrix_size[layer-1][2];
                p[l][1] = k*matrix_size[layer-1][2];
                l++;
            }
        }
    }
    for(j=0;j<height;j++) {
        for(k=0;k<width;k++) {
            for(n=0;n<pix_ref;n++) {
                if(j+p[n][0] >= 0 && j+p[n][0] < height && k+p[n][1] >= 0 && k+p[n][1] < width) {
                    for(m=0;m<matrix_size[layer-1][0];m++) {
                        c[matrix_size[layer-1][0]*n + m] = b[i][matrix_size[layer-1][0]*((j+p[n][0])*width+(k+p[n][1]))+m];
                    }
                }else{
                    for(m=0;m<matrix_size[layer-1][0];m++) {
                            c[matrix_size[layer-1][0]*n + m] = -1.0;
                    }
                }
            }
            vector_x_matrix(c,pix_ref*matrix_size[layer-1][0],a[layer-1],pix_ref*matrix_size[layer-1][0],matrix_size[layer-1][1],
                            c+pix_ref*hidden_channels,matrix_size[layer-1][1]);
            for(n=0;n<matrix_size[layer-1][1];n++) {
                c[pix_ref*hidden_channels+n] += a[layer-1][pix_ref*matrix_size[layer-1][0]][n];
                b[i+1][matrix_size[layer-1][1]*(j*width+k)+n] = c[pix_ref*hidden_channels+n];
            }
        }
    }
    i++;
...
int gene_generator(RealNumber **a,RealNumber **b,int *parent_gene_idx,RealNumber *parent_gene_diff) {
    ...
    int64_t *srdm;
    RealNumber **temp;
    RealNumber alpha,beta,gamma,*tiger;
    ret_code = 0;
    element_size = 0;
    for(i=0;i<layer;i++) {
    ...
    temp = (RealNumber **)malloc((num_parent+3)*sizeof(RealNumber *));
    if(temp == NULL) {
        ret_code = -1;
        goto malloc_temp_error1;
    }
    temp[0] = (RealNumber *)malloc((num_parent+3)*element_size*sizeof(RealNumber));
    if(temp[0] == NULL) {
        ret_code = -1;
        goto malloc_temp_error2;
    }
    for(i=1;i<num_parent+3;i++) {
    ...
    for(j=0;j<element_size;j++) {
        temp[num_parent+2][j] = temp[num_parent+2][j] / beta;
    }
...
int get_filedata(char *filename,RealNumber **a,int *width,int *heigth) {
    int i,fd,ret_code;
    unsigned char *filedata,*size;
    struct stat file_stat;
    uint64_t lc_width,lc_heigth;
    ret_code = 0;
    if(filename == NULL || a == NULL || width == NULL) {
        goto input_error;
    }
    if(*a != NULL) {
        free(*a);
    ...
    if(fstat(fd,&file_stat)) {
        ret_code = -1;
        goto file_fstat_error;
    }
    if(file_stat.st_size <= 2*sizeof(uint64_t) || (file_stat.st_size-(2*sizeof(uint64_t)))%input_datasize != 0) {
        ret_code = -1;
        goto file_file_length_error;
    }
    filedata = (unsigned char *)malloc(file_stat.st_size*sizeof(unsigned char));
    if(filedata == NULL ){
        ret_code = -1;
        goto malloc_filedata_error;
    }
    i=read(fd,filedata,file_stat.st_size);
    if(i != file_stat.st_size) {
        ret_code = -1;
        goto read_error;
    }
    size = (unsigned char *)(&lc_width);
    for(i=0;i<sizeof(uint64_t);i++) {
        size[i] = filedata[file_stat.st_size -(2*sizeof(uint64_t)) + i];
    }
    size = (unsigned char *)(&lc_heigth);
    for(i=0;i<sizeof(uint64_t);i++) {
        size[i] = filedata[file_stat.st_size -(1*sizeof(uint64_t)) + i];
    }
    if(lc_heigth*lc_width*input_datasize+(2*sizeof(uint64_t)) != file_stat.st_size) {
        ret_code = -1;
        goto read_error;
    }
    
    *a = (RealNumber *)malloc(lc_heigth*lc_width*input_datasize*sizeof(RealNumber));
    if(*a == NULL) {
        ret_code = -1;
    }else{
        for(i=0;i<lc_heigth*lc_width*input_datasize;i++) {
            a[0][i] = (RealNumber)filedata[i];
        }
        *width = (int)lc_width;
        *heigth = (int)lc_heigth;
    }
    read_error:
    free(filedata);
    malloc_filedata_error:
    file_file_length_error:
    file_fstat_error:
    close(fd);
    file_open_error:
    input_error:
...
int main() {
    ...
    for(i=0;i<num_parent;i++) {
        ...
        j=read(fd,parent_gene[i],file_stat.st_size);
        close(fd);
        if(j != file_stat.st_size) {
            printf("c\n");
            ret_code = -1;
            goto file_error;
        }
    ...
    while(1) {
        ...
        for(i=0;i<num_data_set;i++) {
            sprintf(file_name,"./data/a%08d.bin",p[i]);
            if(get_filedata(file_name,data[i] + 0,file_size[i] + 0,file_size[i] + 1)) {
                ret_code = -1;
                goto loop_error;
            }
            sprintf(file_name,"./data/b%08d.bin",p[i]);
        ...
        printf("%f\n",f_diff);
        for(i=0;i<num_parent/2;i+=2) {
            for(j=0;j<element_size;j++) {
                parent_gene[parent_gene_idx[num_parent -1 -i]][j] = child_gene[child_gene_idx[i]][j];
            }
            sprintf(file_name,"./gene/gene%08d.bin",parent_gene_idx[num_parent -1 -i]);
            save_gene(file_name,parent_gene[parent_gene_idx[num_parent -1 -i]],element_size);
            f_diff = parent_gene_diff[parent_gene_idx[num_parent -1 -i]] / ((RealNumber)num_data_set);
            g_diff = child_gene_diff[child_gene_idx[i]]  / ((RealNumber)num_data_set);
            printf("%f -> %f\n",f_diff,g_diff);
            parent_gene_diff[parent_gene_idx[num_parent -1 -i]] = child_gene_diff[child_gene_idx[i]];
        }
        R_sort(parent_gene_diff,parent_gene_idx,num_parent);
        f_diff = parent_gene_diff[parent_gene_idx[0]] / ((RealNumber)num_data_set);
        printf("\ngene%08d.bin\n%f\n\n",parent_gene_idx[0],f_diff);

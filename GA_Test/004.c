...
int calc(RealNumber *gene,struct st_data *data,RealNumber *diff,RealNumber ***a,RealNumber **b,RealNumber *c) {
    ...
    for(j=0;j<height*width*matrix_size[layer-1][1];j++) {
        b[i][j] -= data->output_image_data[j];
        temp += b[i][j]*b[i][j];
    }
    if(isfinite(temp)) {
...
int get_filedata(struct st_data *data,int file_nunber) {
    ...
    data->width = internal_width[0];
    data->height = internal_height[0];
    data->input_image_data = internal_data[0];
    internal_data[0] = NULL;
    data->output_image_data = internal_data[1];
...
void* calc_p(void *arg) {
    ...
    b[0] = (RealNumber *)malloc(2*hidden_channels*(data->width)*(data->height)*sizeof(RealNumber));
    if(b[0] == NULL) {
        goto malloc_b_error2;
    }
    b[1] = b[0] + hidden_channels*(data->width)*(data->height);
    for(i=2;i<layer+1;i++) {
        b[i] = b[i&0x01];
    }
...
void* calc_subset(void *arg) {
    ...
    int i,j,k,l,q,element_size;
    uint64_t *urnd;
    RealNumber ***a,**b,*c,*diff,*temp_gene[2],temp_diff;
    ...
    b[0] = (RealNumber *)malloc(2*hidden_channels*k*sizeof(RealNumber));
    if(b[0] == NULL) {
        goto malloc_b_error2;
    }
    b[1] = b[0] + hidden_channels*k;
    for(i=2;i<layer+1;i++) {
        b[i] = b[i&0x01];
    }
    c = (RealNumber *)malloc(2*pix_ref*hidden_channels*sizeof(RealNumber));
    ...
    diff = (RealNumber *)malloc((num_group_size + num_generate)*sizeof(RealNumber));
    if(diff == NULL)
    {
        goto malloc_diff_error;
    }
    urnd = (uint64_t *)malloc((num_group_size + num_generate)*(sizeof(uint64_t) + 2*sizeof(int)));
    if(urnd == NULL)
    {
        goto malloc_urnd_error;
    }
    gene_index[1] = (int *)(urnd + num_group_size + num_generate);
    gene_index[2] = gene_index[1] + (num_group_size + num_generate);
    temp_gene[0] = (RealNumber *)malloc(2*element_size*sizeof(RealNumber));
    if(temp_gene[0] == NULL)
    {
    ...
    q = (num_group_size + num_generate) / 2;
    
    for(i=0;i<(num_group_size + num_generate);i++)
    {
        diff[i] = 0.0;
    }
    for(i=0;i<(num_group_size + num_generate);i++)
    {
        ...
        {
            calc(gene[i],data + j,diff + i,a,b,c);
        }
    }

    for(l=0;l<(num_group_size + num_generate)*3;l++)
    {
        R_sort_p(diff,gene_index,(num_group_size + num_generate));
        gene_index[1][0] = 0;
        gene_index[2][0] = num_group_size + num_generate;
        for(i=1;i<(num_group_size + num_generate);i++)
        {
            gene_index[1][i] = i;
            gene_index[2][i] = gene_index[2][i-1] + num_group_size + num_generate -i;
        }
        ...
        for(i=0;i<q;i++)
        {
            urnd[i] %= gene_index[2][(num_group_size + num_generate)-1-i];
            for(j=0;j<(num_group_size + num_generate)-1-i;j++)
            {
            ...
            for(;j<(num_group_size + num_generate)-1-i;j++)
            {
                gene_index[1][j] = gene_index[1][j+1];
                gene_index[2][j] = gene_index[2][j+1] - num_group_size + num_generate -urnd[i];
            }
        }
        ...
        for(i=0;i<q-1;i++)
        {
            ...
            {
                temp_gene[0][j] += gene[gene_index[1][i]][j];
            }
        }
        for(i=0;i<element_size;i++)
        {
            temp_gene[0][i] /= (RealNumber)(q-1);
            temp_gene[1][i] = 2.0*temp_gene[0][i] - gene[gene_index[1][q-1]][i];
        }
        temp_diff = 0.0;
        for(j=0;j<num_data_set;j++)
        {
            calc(temp_gene[1],data + j,&temp_diff,a,b,c);
        }
        j=0;
        ...
        if(j == 1)
        {
            for(i=0;i<element_size;i++)
            {
                gene[gene_index[1][q-1]][i] = temp_gene[1][i];
            }
        ...
        for(i=0;i<element_size;i++)
        {
            temp_gene[1][i] = 0.5*temp_gene[0][i] + 0.5*gene[gene_index[1][q-1]][i];
        }
        temp_diff = 0.0;
        for(j=0;j<num_data_set;j++)
        {
...
int subset_gene_calc(RealNumber **superset_gene,int *superset_idx,char *file_name)
{
    int i,j,k,l,m,n,ret_code,element_size,grupe_num;
    RealNumber ***subset_gene;
    double *temp_gene[3];
    struct st_data **data;
    int *threadnum,*thread_grupe_num,*file_index,**subset_idx;
    void ***lp;
    pthread_t *threads;
    ret_code = 0;
    element_size = 0;
    ...
    data = (struct st_data **)malloc(ThreadQuantity*sizeof(struct st_data *));
    if(data == NULL)
    {
        ret_code = -1;
        goto error01;
    ...
    for(i=0;i<ThreadQuantity;i++)
    {
        data[i] = (struct st_data *)malloc(num_data_set*sizeof(struct st_data));
        if(data[i] == NULL)
        {
            ret_code = -1;
            goto error02;
        ...
        for(j=0;j<num_data_set;j++)
        {
            data[i][j].input_image_data = NULL;
            data[i][j].output_image_data = NULL;
            data[i][j].width = 0;
            data[i][j].height = 0;
        }
    }
    ...
    for(i=0;i<ThreadQuantity;i++)
    {
        subset_gene[i] = (RealNumber **)malloc((num_group_size + num_generate)*sizeof(RealNumber *));
    ...
    for(i=0;i<ThreadQuantity;i++)
    {
        subset_gene[i][0] = (RealNumber *)malloc(element_size*(num_group_size + num_generate)*sizeof(RealNumber));
        if(subset_gene[i][0] == NULL)
        {
            ret_code = -1;
            goto error04;
        }
        for(j=1;j<(num_group_size + num_generate);j++)
        {
    ...
    for(i=0;i<ThreadQuantity;i++)
    {
        subset_idx[i] = (int *)malloc((num_group_size + num_generate)*sizeof(int));
        if(subset_idx[i] == NULL)
        {
            ret_code = -1;
            goto error05;
        }
        for(j=0;j<(num_group_size + num_generate);j++)
        {
            subset_idx[i][j] = j;
        }
    }
    thread_grupe_num = (int *)malloc((ThreadQuantity + data_pool_size)*sizeof(int));
    if(thread_grupe_num == NULL)
    {
        ret_code = -1;
        goto error05;
    }
    file_index = thread_grupe_num + ThreadQuantity;
    threadnum = (int *)malloc(ThreadQuantity*sizeof(int));
    if(threadnum == NULL)
    {
        ret_code = -1;
    ...
    temp_gene[0] = (double *)malloc(3*element_size*sizeof(double));
    if(temp_gene[0] == NULL)
    {
        ret_code = -1;
        goto error09;
    ...
    for(i=0;i<grupe_num;i++)
    {
        ...
        if(m==-1)
        {
            ...
            for(j=0;j<num_group_size;j++)
            {
                ...
                sprintf(file_name,"./gene/gene%08d.bin",superset_idx[grupe_num*j + thread_grupe_num[n]]);
                save_gene(file_name,superset_gene[superset_idx[grupe_num*j + thread_grupe_num[n]]],element_size);
            }
        }
        for(k=0;k<element_size;k++)
        {
        ...
        for(j=0;j<num_group_size;j++)
        {
            for(k=0;k<element_size;k++)
            {
                ...
                temp_gene[0][k] += (double)subset_gene[n][j][k]; 
                temp_gene[1][k] += ((double)subset_gene[n][j][k]) * ((double)subset_gene[n][j][k]); 
            }
        }
        for(k=0;k<element_size;k++)
        {
            temp_gene[0][k] /= (double)num_group_size;
            temp_gene[1][k] /= (double)num_group_size;
            temp_gene[1][k] -= temp_gene[0][k]*temp_gene[0][k];
            temp_gene[1][k] = 3.0*sqrt(temp_gene[1][k]);
            if(!isfinite(temp_gene[0][k]))
            {
                temp_gene[0][k] = 0.0;
            }
            if(!isfinite(temp_gene[1][k]))
            {
                temp_gene[1][k] = 1.0;
            }
        }
        pthread_mutex_lock(&rand_mutex);
        for(j=num_group_size;j<(num_group_size + num_generate);j++)
        {
            get_sd(temp_gene[2],element_size);
            for(k=0;k<element_size;k++)
            {
                temp_gene[2][k] *= temp_gene[1][k];
                temp_gene[2][k] += temp_gene[0][k];
                subset_gene[n][j][k] = temp_gene[2][k];
            }
        }
        if(l + num_data_set >= data_pool_size)
        {
            random_list(file_index,data_pool_size);
            l=0;
        }
        ...
        for(j=0;j<num_data_set;j++)
        {
            if(get_filedata(data[n] + j,file_index[l]))
            {
                ret_code = -1;
                goto error10;
            }
    ...
    for(i=0;i<ThreadQuantity;i++)
    {
        if(data[i] != NULL)
        {
            for(j=0;j<num_data_set;j++)
            {
                if(data[i][j].input_image_data != NULL)
                {
                    free(data[i][j].input_image_data);
                }
                if(data[i][j].output_image_data != NULL)
                {
                    free(data[i][j].output_image_data);
                }
            }
            free(data[i]);
        }
...
int main()
{
    int i,j,k,ret_code,element_size,*superset_idx,*file_index;
    ...
    for(i=0;i<num_parent;i++)
    {
        ...
        if(load_gene(file_name,superset_gene[i],element_size))
        {
            ret_code = -1;
            goto error09;
        }
    ...
    while(1)
    {
        ...
        for(i=0;i<num_parent;i++)
        {
            printf("%f\n",255.0*255.0*superset_diff[i]/((RealNumber)num_data_set));
        }
        printf("\n\n\n");

...
#endif
#define num_group 100
#define num_parent 300
#define num_generate 300
#define num_data_set 3
...
int gene_generator(RealNumber **a,RealNumber **b,int *parent_gene_idx) {
    ...
    temp[0] = (double *)malloc(4*element_size*sizeof(double));
    if(temp[0] == NULL)
    {
        ret_code = -1;
        goto malloc_temp_error;
    }
    for(i=1;i<4;i++)
    {
    ...
    {
        temp[0][j] = a[parent_gene_idx[1]][j] - a[parent_gene_idx[0]][j];
        temp[1][j] = a[parent_gene_idx[2]][j] - a[parent_gene_idx[0]][j];
        
        alpha += temp[0][j]*temp[0][j];
        beta += temp[1][j]*temp[1][j];
        gamma += temp[0][j] * temp[1][j];
    }
    gamma *= gamma;
    delta = sqrt(beta - gamma/alpha);
    if(!isfinite(delta))
    {
        delta = 0.0;
    }
    printf("delta=%f\n",delta);
    alpha = 0.5;
    beta = 0.35/sqrt(((double)element_size));

    for(i=0;i<num_generate;i++)
    {
        get_sd(temp[3],element_size);
        get_sd(&gamma,1);
        gamma *= alpha;
        for(j=0;j<element_size;j++)
        {
            temp[3][j] *= delta*beta;
            temp[3][j] += (0.5+gamma)*a[parent_gene_idx[1]][j] + (0.5-gamma)*a[parent_gene_idx[0]][j];
        }
        for(j=0;j<element_size;j++)
        {
            b[i][j] = temp[3][j];
...
int main() {
    ...
    while(1) {
        if(k == num_group)
        {
            random_list(parent_gene_idx,num_parent);
            k=0;
        }
        ...
        gene_generator(parent_gene,child_gene,parent_gene_idx + k*3);
        for(i=0;i<num_generate;i++) {
        ...
        for(i=0;i<2;i++) {
            for(j=0;j<element_size;j++) {
                parent_gene[parent_gene_idx[k*3 + i]][j] = child_gene[child_gene_idx[i]][j];
            }
            sprintf(file_name,"./gene/gene%08d.bin",parent_gene_idx[k*3 + i]);
            save_gene(file_name,parent_gene[parent_gene_idx[k*3 + i]],element_size);
            g_diff = child_gene_diff[child_gene_idx[i]]  / ((RealNumber)num_data_set);
            g_diff *= 255.0*255.0;
            printf("gene%08d.bin  %f\n",parent_gene_idx[k*3 + i],g_diff);
        }
        printf("\n");
        k++;

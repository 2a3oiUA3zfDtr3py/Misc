...
int gene_generator(RealNumber **a,RealNumber **b,int *parent_gene_idx,RealNumber *parent_gene_diff) {
    ...
    alpha = parent_gene_diff[parent_gene_idx[0]]/parent_gene_diff[parent_gene_idx[num_parent-1]];
    if(!isfinite(alpha) || parent_gene_diff[parent_gene_idx[0]] < 0.0 || parent_gene_diff[parent_gene_idx[num_parent-1]] < 0.0)
    {
        alpha = 1.2;
        delta = sqrt(0.5);
    }
    ...
    {
        alpha = pow(2.0,(alpha - 0.5));
        delta = sqrt(2.0);
        delta = (delta - alpha)/(delta - 1.0/sqrt(2.0));
        delta = delta*0.5 + 0.5;
    }
    if(alpha < 0.8)
    {
...
int main() {
    ...
    while(1) {
        ...
        for(i=0;i<num_parent;i++) {
            for(j=0;j<element_size;j++) {
                parent_gene[i][j] = child_gene[child_gene_idx[i]][j];
            }
            sprintf(file_name,"./gene/gene%08d.bin",i);
            save_gene(file_name,parent_gene[i],element_size);
            f_diff = parent_gene_diff[i] / ((RealNumber)num_data_set);
            f_diff *= 255.0*255.0;
            g_diff = child_gene_diff[child_gene_idx[i]]  / ((RealNumber)num_data_set);
            g_diff *= 255.0*255.0;
            printf("%f -> %f\n",f_diff,g_diff);
            parent_gene_diff[i] = child_gene_diff[child_gene_idx[i]];
        }
        R_sort(parent_gene_diff,parent_gene_idx,num_parent);
